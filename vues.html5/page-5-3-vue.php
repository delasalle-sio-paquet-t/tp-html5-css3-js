...
<p><i>(8 caractères minimun avec au moins un chiffre, une lettre minuscule et une lettre majuscule)</i></p>

<form id="formModificationMdp" name="formModificiationMdp" method="post" action="#">
<p>
<label for="txtNouveauMdp">Nouveau mot de passe :</label>
<input type="<?php if($afficherMdp == "off") echo "password"; else echo "text";?>" id="txtNouveauMdp" name="txtNouveauMdp" required pattern="^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9]).{8,}$" value="<?php echo $nouveauMdp; ?>"/>
</p>
<p>
<label for="txtConfirmation">Confirmation :</label>
<input type="<?php if($afficherMdp == "off") echo "password"; else echo "text";?>" id="txtConfirmation" name="txtConfirmation" required pattern="^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9]).{8,}$" value="<?php echo $confirmationMdp; ?>"/>
</p>
<p>
<label for="caseAfficherMdp">Afficher en clair :</label>
<input type="checkbox" id="caseAfficherMdp" name="caseAfficherMdp" <?php if($afficherMdp == 'on') echo 'checked'; ?>/>
</p>
<p>
